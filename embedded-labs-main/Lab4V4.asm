...
;OCR0A will always be 10k
;OCR0B alters the duty cycle
;OC0B will be the power pin
...
;ISRs---------------------------------------------------
PCINT0_ISR: ;channel b of RPG (left)-------------------------------
...
PCINT1_ISR: ;channel a of RPG (right)-------------------------------
.set DC_value = DC_value+1
rcall DCtoString
rcall lineRestart
rcall send_DC_Message
...
PCINT2_ISR: ;the push button-------------------------------
/*sbis portd,5 ;skip if the led is on
rjmp LED_is_off

sbic portd,5; skip if the led is off
rjmp LED_is_on
...
TIMER1_COMPA:
;output compare A (top of counter)
ldi tmp1, 0b00100011
out OCR0A, tmp1
in tmp1,OCR0A
out portd2,tmp1
;output compare B (counter change)
;ldi tmp1, 0b00100011
;out OCR0B, r16
RETI
;TCCR0A
...
DCtoString:
.set tens=(DC_value/10)
.set ones=(DC_value%10)
.set tens_message=(tens+48)
.set ones_message=(ones+48)
DC_msg1: .db "DC=",tens_message,ones_message,"%" ;6 chars
.dw 0
...
send_DC_Message:
ldi r30,LOW(2*DC_msg1) ; Load Z register low
ldi r31,HIGH(2*DC_msg1) ; Load Z register high
ldi r28,6
rcall displayCString
ret
